...
9. **Linguagem clara — sem sopa de siglas.** O relatório é lido por executivos, não por quem aplicou a survey. NUNCA escreva códigos sozinhos (D4, O9, T9) nem "=2". Use sempre o **nome por extenso** da dimensão (glossário em `references/dimension-names.md`), com o código entre parênteses só para rastreabilidade, e traduza scores como **"nível X de 5"**. Cada gap deve ser explicado em uma frase que um leigo entenda — o número é evidência, não a mensagem. **Sempre inclua a seção Glossário das Siglas ao final**, para o relatório ser autoexplicativo.
...
- Tabela com os códigos citados (D, O, T) e seus nomes por extenso + escala de níveis + termos técnicos (RAG, POC, fine-tuning, FinOps, AI-DLC). Extraia de `references/dimension-names.md`. Sempre incluir — é o que torna o relatório autoexplicativo.
...
- `references/dimension-names.md` — glossário de nomes legíveis das dimensões (use SEMPRE no relatório).
